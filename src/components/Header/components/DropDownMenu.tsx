  ...
  return (
    <>
      ...
      >
        {data.name === 'Change' ? (
          <>
            <img src="/images/logo3.png" alt="menu icon" onMouseEnter={handleToggle} onMouseLeave={handleClosePopper} />
          </>
        ) : data.name === 'Sign Out' ? (
          <span className={classes.userName} onMouseEnter={handleToggle} onMouseLeave={handleClosePopper}>
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                  {data.children.map((child: ListProps, key: number) => {
                    if (child.name === 'ZNX Mainnet' || child.name === 'ZNX Testnet') {
                      return (
                        <a key={key} href={`${child.path}`} className={classes.menuListLink}>
                          <MenuItem
                      ...
                      return (
                        <MenuItem className={classes.menuList} key={key}>
                          <Button key={key} href="/login?cmd=logout" className={classes.signoutBtn} disableRipple>
                            Sign Out
                          </Button>
                        </MenuItem>
                      )
